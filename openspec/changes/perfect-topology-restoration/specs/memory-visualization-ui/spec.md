...
- **Pass 2** SHALL sequentially generate edges and internal Handle bindings, strictly utilizing the fully-populated registry to guarantee that forward-pointing references (e.g. `next`, `left`, `right`) are never aborted due to missing IDs.
